...
rebar_opts(RebarFile) ->
    ...
    case file:consult(RebarFile) of
        {ok, Terms} ->
            ...
            %% FIXME(lga): Made quickfix here since adding the lager parser to erl-opts breaks this synastic plugin
            lists:delete({parse_transform, lager_transform}, proplists:get_value(erl_opts, Terms, [])) ++ IncludeDeps;
